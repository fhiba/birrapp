 ...
 *
 * El líquido va en `--birra` —el ámbar quemado de heritage— y no en el acento.
 * Es la única ilustración de la app —todo lo demás que lleva color es un dato
 * o un control— y con el acento en hueso quedaba líquido blanco debajo de
 * espuma blanca: un vaso vacío.
 *
 * La espuma y el vidrio del vaso tenían el hueso viejo escrito a mano
 * (`#FFFFFF`, `rgba(244,245,247,.55)`), así que la pinta se quedó en la paleta
 * anterior cuando la app cambió de fondo: espuma fría sobre espresso cálido.
 */
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center' }}>
        <svg width="58" height="76" viewBox="0 0 58 76" aria-hidden>
          ...
          <g clipPath="url(#glass)">
            ...
            <g>
              ...
              <rect x="0" width="58" height="5" fill="var(--cream)" y="60">
                <animate attributeName="y" values="66;12;66" dur="3.8s"
                  calcMode="spline" keySplines=".4 0 .2 1;.4 0 .2 1" repeatCount="indefinite" />
              </rect>
            </g>
            {[[22, 0], [34, 1.1], [28, 2.2]].map(([cx, delay], i) => (
              <circle key={i} cx={cx} r="1.8" fill="var(--cream)" cy="70">
        ...
        <p style={{ color: 'var(--muted)', fontSize: 'var(--t-3)', marginTop: 'var(--s-4)' }}>{message}</p>
      </div>
    </div>
  )
}
