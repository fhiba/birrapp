...
import news.inkan.birrapp.core.ApiException
import news.inkan.birrapp.prices.NewPriceRequest
...
class PriceReportTest {
    ...
    fun `un precio atipico queda retenido y no se publica`() {
        ...
        val detail = news.inkan.birrapp.bars.BarRepo(TestDb.db).detail(bar, null, null)!!
        ...
        val flags = news.inkan.birrapp.moderation.ModerationRepo(TestDb.db).openFlags()
    ...
    fun `sigue igual reinserta el precio vigente con fecha de hoy`() {
        ...
        val antes = news.inkan.birrapp.bars.BarRepo(TestDb.db).detail(bar, null, null)!!
        assertEquals("aging", antes.prices.single().freshness)

        val otro = TestDb.insertUser("confirmador")
        repo.confirm(bar, "rubia", otro)

        val despues = news.inkan.birrapp.bars.BarRepo(TestDb.db).detail(bar, null, null)!!
